...
if "messages" not in st.session_state:
    ...
    # 英語の回答精度向上のため gpt-4o-mini ではなく gpt-4o に変更
    st.session_state.llm = ChatOpenAI(model_name="gpt-4o", temperature=0.5)
    # st.session_state.llm = ChatOpenAI(model_name="gpt-4o-mini", temperature=0.5)

    # 回答精度向上のため max_token_limit を 1000 → 3000 に変更
    # ↓重要なコンテキストが短縮されすぎると精度が落ちる場合があるため
...
if st.session_state.start_flg:
    ...
    if st.session_state.mode == ct.MODE_3 and (
        st.session_state.dictation_button_flg or 
        ...
        # チャット入力時の処理
            ...
            st.session_state.messages.append({
                "role": "assistant", 
                "content": llm_response_evaluation
                })
            st.session_state.messages.append({
                "role": "other"
                })
    ...
    if st.session_state.mode == ct.MODE_2 and (
        st.session_state.shadowing_button_flg or 
